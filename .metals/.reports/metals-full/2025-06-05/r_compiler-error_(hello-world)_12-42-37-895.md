...
object Main extends JFXApp3 {
  ...
  override def start(): Unit = {
    ...
    def barChartScene(stage: Stage, database: Database): Scene = {
      ...
      try {
        val barChartData = database.getBarChartData(year.value.value)
        val series = new chart.XYChart.Series[String, Number] {
        name = "Total by Category"
        barChartData ++ barChartData.map { case (cat, amt) =>
            XYChart.Data[String, Number](cat, amt)
          }.toSeq:_*
        }

        val barChart = new chart.BarChart(chart.CategoryAxis("Month"), chart.NumberAxis("Amount"), ObservableBuffer(@@)) {
          title = "Spending by Category"
        }
