error id: file:///C:/Users/Jayso/Projects_For_Github/pls/hello-world/src/main/scala/app/Main.scala:
...
object Main extends JFXApp3 {
  ...
  override def start(): Unit = {
    ...
    lazy val analyze_screen: scalafx.scene.Scene{} = new Scene{
      ...
      val monthData = ObservableBuffer(
        data.map { case (month, amount) =>
      ...
      val barChart = new chart.PieChart(chartData) { 
        title = "Spending by Month"
        visible = false
      }

      val charts = Seq(pieChart, barChart)
      ...
      content = new VBox{
        ...
        children = Seq(new Label("Charts"), new ComboBox[String](tables) { 
          ...
          onAction = _ => {
            ...
            }}, pieChart, barChart , new Button("HOME") { onAction = _ => stage.scene = home_screen})
      }
    }


    def barChartScene(stage: Stage, database: Database): Scene = {
      val year = new ComboBox[String](database.grab_years())
      try {
        val data = database.getBarChartData(year.value.value)
        val series = new XYChart.Series[String, Number] {
        name = "Total by Category"
        data ++= data.map { case (cat, amt) =>
            XYChart.Data[String, Number](cat, amt)
          }
        }

        val barChart = new BarChart(CategoryAxis("Month"), NumberAxis("Amount")) {
          title = "Spending by Category"
          data = ObservableBuffer(series)
        }

        val backButton = new Button("Back") {
          onAction = _ => stage.scene = homeScene(stage, db)
        }

        new Scene {
          root = new VBox(10) {
            padding = scalafx.geometry.Insets(20)
            children = Seq(barChart, backButton)
          }
        }
      } catch {
        @@new Alert("i")
