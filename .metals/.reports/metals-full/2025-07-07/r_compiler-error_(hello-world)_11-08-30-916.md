error id: 58CEC3EB035BF88103B4F4C28A62F80F
...
// yuh
  ...
  override def start(): Unit = {
          ...
          new VBox {
            ...
            children = Seq(
              ...
              new Button("Assistant") { style = homeButtonStyle
                onAction = _ => stage.scene = assistantScene(stage, database, months, years)
    ...
    def assistantScene(stage: Stage, database: Database, months: Seq[String], years: List[String]): Scene = {
      val year_Data = years.flatMap { y =>
              database.viewYear(y, "_").map { expense =>
                List(expense.month, expense.category, expense.amount.toString).mkString(",")
              }.map(data => s"transaction for $y: $data")
            }.mkString(", ")
      ...
      val sendButton = new Button("Send")
            // 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
      ...
      def sendMessageToAI(message: String): Unit = {
        ...
        Future {
          val apiToken = "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" // Replace with your actual token
          val model = "mistralai/Mixtral-8x7B-Instruct-v0.1"
          val url = new URL(s"https://router.huggingface.co/together/v1/chat/completions")
          ...
          try {
            ...
            writer.write(payload.render())
            writer.flush()
            writer.close()

            val responseCode = conn.getResponseCode
            val inputStream = if (responseCode == 200) conn.getInputStream else conn.getErrorStream
            val responseStr = scala.io.Source.fromInputStream(inputStream).mkString
            inputStream.close()

            //Parse content (very basic parsing; consider using a JSON parser like uPickle or circe)
            val json = ujson.read(responseStr)
            val answer = json.arr.head.obj("content").str
            ...
            case e: Exception =>
              Platform.runLater {
                chatArea.appendText(s"Error: ${e.getMessage}\n")
              }
          } finally {
            conn.disconnect()
          }
