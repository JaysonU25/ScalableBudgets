error id: file:///C:/Users/Jayso/Projects_For_Github/pls/hello-world/src/main/scala/app/Main.scala:local5
file:///C:/Users/Jayso/Projects_For_Github/pls/hello-world/src/main/scala/app/Main.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -scalafx/Includes.month.
	 -scalafx/scene/control/month.
	 -scalafx/beans/property/month.
	 -month.
	 -scala/Predef.month.
offset: 6344
...
object Main extends JFXApp3 {
  ...
  override def start(): Unit = {
    ...
    val months_list = (Seq("January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"))
    val selected_month = new Label()
    val switch_to_table = new Button("Add a table")
    val switch_to_expenses = new Button("Add a Expense")
    val switch_to_home = new Button("Go back home")
    ...
    lazy val home_screen: Scene = new Scene{
      content = new VBox {
        spacing = 10
        children = Seq(new Label("Scalable Budgets"),new Button("View Expenses") { onAction = _ => stage.scene = view_expenses},new Button("AddTable") { onAction = _ => stage.scene = new_table}, new Button("AddExpenses") { onAction = _ => stage.scene = new_expense_screen})
      } 
    }
    ...
    lazy val new_expense_screen: Scene = new Scene{
        content = new VBox { 
        spacing = 10
        children = Seq(new Label("Add an Expense"), new Label("Which year did it fall under:"), new ComboBox[String](tables) { promptText = "Select a Budget Year"
        onAction = _ => {selected_year.text = this.value.value} }, new Label("Which month did it fall under:"), month, new Label("What was the expense:"), user_category, new Label("Amount:"),user_expense,new_expense, new Button("HOME") { onAction = _ => stage.scene = home_screen})
      }
    }
    ...
    lazy val view_expenses: Scene = new Scene{
        ...
        children = Seq(new Label("View your expenses"), new Label("Which year would you like to view:"), new ComboBox[String](tables) { 
          ...
          onAction = _ => {
            ...
            selected_year.text = selected
            val expenses = database.viewYear(selected, Option(mont@@h.value.value).getOrElse("."))
            table.items = toFX(expenses)
            } 
          }, 
          new Label("Which month did it fall under:"),  new button, table, new Button("HOME") { onAction = _ => stage.scene = home_screen})
      }
    }
